<h1 align="center">Hi 👋, I'm Mohamed Abo El Ala</h1>
<h3 align="center">Junior .NET Backend Developer | Full-Stack Background</h3>

---

### 👨‍💻 About Me
- 🎓 BSc in Computer & Information Science – Zagazig University (GPA: 3.3)
- 🧠 ITI Graduate – Full Stack .NET (Intake 2025)
- 🔭 Currently working on **Booking System**
- 🌱 Learning **Angular** & improving frontend skills
- 💼 Freelance Backend Developer (ASP.NET Core)
- 📫 Reach me at: **mo2.prog@gmail.com**

---

### 🛠️ Tech Stack
**Backend**
- C#, ASP.NET Core, Web API
- Entity Framework Core, LINQ
- SQL Server, MySQL
- REST APIs, JWT Authentication

**Frontend**
- HTML, CSS, JavaScript
- TypeScript, Angular
- Bootstrap, Razor Pages

**Tools & Concepts**
- Git & GitHub
- Docker
- OOP, SOLID Principles
- Clean Architecture, MVC
- Debugging & Performance Optimization

---

### 🚀 Featured Projects
- 🏨 **Hotel Booking System**  
  ASP.NET Core Web API, EF Core, SQL Server, Angular, JWT  
  🔗 https://github.com/MoHamedAboElAla/HotelBookingApi<br>
  🔗 https://github.com/MoHamedAboElAla/HotelBookingAngular


- 📚 **Book Store MVC**  
  ASP.NET Core MVC, Identity, Stripe API  
  🔗 https://github.com/MoHamedAboElAla/BookStoreMVC

- 🎮 **Game Zone**  
  ASP.NET Core, JavaScript, jQuery, Ajax  
  🔗 https://github.com/MoHamedAboElAla/GameZone

- 🏬 **Electronics Store**  
  ASP.NET Core MVC, EF Core, Stripe  
  🔗 https://github.com/MoHamedAboElAla/Electronics_Store

- 🏡 **Villa API**  
  ASP.NET Core Web API, EF Core, AutoMapper  
  🔗 https://github.com/MoHamedAboElAla/VillaApi

---

### 🤝 Connect with Me
<p align="left">
<a href="https://www.linkedin.com/in/aboelala/" target="_blank">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/linkedin/linkedin-original.svg" width="30"/>
</a>
<a href="https://github.com/MoHamedAboElAla" target="_blank">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/github/github-original.svg" width="30"/>
</a>
<a href="https://codeforces.com/profile/mohamedaboelala" target="_blank">
  <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/codeforces.svg" width="30"/>
</a>
</p>
